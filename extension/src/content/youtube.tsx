...
function mergeTranscriptCues(
  ...
  for (let i = 1; i < sorted.length; i++) {
    ...
    // Only check sentence endings for manual subtitles
    // Auto-generated subtitles often have inconsistent punctuation
    const shouldMerge =
      isConsecutive &&
      isShortDuration &&
      (isAutoGenerated ? true : !hasSentenceEnd);
  ...
  useEffect(() => {
    const checkUrl = () => {
      ...
      if (window.location.pathname !== "/watch") {
        if (videoId !== null) {
          console.log("[LingText] Leaving /watch, deactivating");
          setVideoId(null);
        }
        return;
      }
      ...
      if (id && id !== videoId) {
        console.log(`[LingText] Video changed on /watch: ${id}`);
        setVideoId(id);
    ...
    try {
      ...
      if (captionTracks && captionTracks.length > 0) {
        ...
        if (track?.baseUrl) {
          ...
          // Detect if it's auto-generated (ASR)
          isAutoGenerated =
            track.kind === "asr" ||
            (track.vssId && track.vssId.startsWith("a."));
          console.log(
            `[LingText] Using track: ${track.languageCode || "unknown"} (${track.kind || "standard"}, ${isAutoGenerated ? "auto-generated" : "manual"})`
          );
        }
      }
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (xmlText.includes("<text")) {
              ...
              if (cues.length > 0) {
                const mergedCues = mergeTranscriptCues(cues, isAutoGenerated);
                console.log(
        ...
        try {
          ...
          if (jsonData.events) {
            ...
            if (jsonCues.length > 0) {
              const mergedCues = mergeTranscriptCues(jsonCues, isAutoGenerated);
              console.log(
