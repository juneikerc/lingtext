...
export async function pickAudioFile(): Promise<FileSystemFileHandle | null> {
  ...
  if (!hasPicker) {
    alert('Tu navegador no soporta el selector de archivos (File System Access API). Usa URL de audio.')
    return null
  }
  const anyWin = window as unknown as {
    showOpenFilePicker?: (options?: OpenFilePickerOptions) => Promise<FileSystemFileHandle[]>
  }
  const [handle] = await (anyWin.showOpenFilePicker as NonNullable<typeof anyWin.showOpenFilePicker>)({
    multiple: false,
    types: [
      {
